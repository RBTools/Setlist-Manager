...
namespace SetlistManager
{
    public partial class SetlistExporter : Form
    {
        ...
        private string FormatDiff(int diff, bool isCSV = false)
        {
        ...
        private void AddHeaderFooter(PdfContents Contents, int page_number)
        {
            ...
            if (chkLink.Checked && !string.IsNullOrEmpty(WebLink))
            {
                Contents.DrawText(NormalFont, footersize, page_width - left_border, top_border - line_height, TextJustify.Right, WebLink);
            }
            ...
            for (var i = 0; i < Songs.Count; i++)
            {
                ...
                foreach (var song in songs)
                {
                    ...
                    try
                    {
                        ...
                        Contents.DrawImage((song.GuitarDiff > 0 ? (!isRB4 && song.ProGuitarDiff > 0 ? proguitar : guitar) : noguitar), instruments_left + spacer, current_height - (line_width * 4), 0.15, 0.15);
                        Contents.DrawImage((song.BassDiff > 0 ? (!isRB4 && song.ProBassDiff > 0 ? probass : bass) : nobass), instruments_left + spacer + 0.17, current_height - (line_width * 4), 0.15, 0.15);
                        ...
                        if (!isRB4)
                        {
                            Contents.DrawImage((song.KeysDiff > 0 ? (song.ProKeysDiff > 0 ? prokeys : keys) : nokeys), instruments_left + (0.17 * 3), current_height - (line_width * 4), 0.15, 0.15);
                        }
        ...
        private void doPDFEspher(PdfDocument Document)
        {
            ...
            for (var i = 0; i < Songs.Count; i++)
            {
                ...
                foreach (var song in songs)
                {
                    var limit = chkLink.Checked || chkCountFooter.Checked || chkPages.Checked ? top_border + line_height : top_border; 
                ...
                if (chkHeadersCSV.Checked)
                {
                    ...
                    if (radioEverythingCSV.Checked)
                    {
                        line = line + del + "\"Track #\"" + del + "\"Master?\"" + del + "\"Year Released\"" + del + "\"Year Recorded\"" + del + 
                            (!isRB4 ? "\"Pro Bass\"" + diff + del + "\"Pro Bass Tuning\"" + del + "\"Pro Guitar\"" + diff + del + 
                ...
                foreach (var song in Songs)
                {
                    ...
                    if (radioEverythingCSV.Checked)
                    {
                        ...
                        line = line + del + "\"" + song.TrackNumber + "\"" + del + "\"" + song.IsMaster() + "\"" + del + "\"" + song.YearReleased + "\"" + del +
                               "\"" + song.YearRecorded + "\"" + 
                               (!isRB4 ? del + "\"" + FormatDiff(song.ProBassDiff, true) + "\"" + del + "\"" + song.ProBassTuning + "\"" + del +
                               ...
                               "\"" + del + "\"" + song.DrumBank + "\"" + del + "\"" + song.SongId + "\"" + del + "\"" + (isCustom ? "No" : "Yes") + "\"" + del + "\"" + 
                               song.FilePath + "\"" : "");
                    }
                    sw.WriteLine(line);
                }
        ...
        private bool ExportExcel(bool CreateTempFile = false)
        {
            ...
            if (radioEverythingExcel.Checked)
            {
                ...
                {
                    colWidths.AddRange(new List<double> { 7, 8, 9, 9, diff_width, 18, diff_width + 1, 18, diff_width, diff_width, 8, 9, 10, 9, 8, 6, 8, 9, 22, 17, 12, 10, 65 });
                }
            }
                ...
                else if (radioEverythingExcel.Checked)
                {
                    ...
                    else
                    {
                        dt1.Rows.Add(new object[]
                        {
                            ...
                            song.PercussionBank, song.DrumBank, song.SongId, isCustom ? "No" : "Yes", song.FilePath
                        });
                    }
                }
            }
        ...
        private void SetlistExport_Shown(object sender, EventArgs e)
        {
            ...
            var link = Application.StartupPath + "\\bin\\link";
            if (File.Exists(link))
            {
                try
                {
        ...
        private void cboPDF_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            else if (ActiveIndex == PDFStyle.DeadlyRobots)
            {
                ...
                chkLink.Checked = !string.IsNullOrEmpty(WebLink);
        ...
        private void DisplayThumbnail()
        {
            ...
            picThumbnail.Image = Tools.LoadImage(jpg);
            Tools.DeleteFile(jpg);
        }
        ...
        private void SetlistExporter_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (!picWorking.Visible)
            {
                ...
                var link = Application.StartupPath + "\\bin\\link";
                using (var sw = new StreamWriter(link, false))
                {
                    sw.Write(WebLink);
                }
                return;
        ...
        private void chkLink_MouseClick(object sender, MouseEventArgs e)
        {
            if (e.Button != MouseButtons.Left)
            if (isLoading) return;
            if (chkLink.Checked)
            {
                WebLink = Interaction.InputBox("Enter footer link:", AppName, WebLink);
